...
public class PhiDichVuController {
    ...
    private final String tenPhi = "PhiDichVu";
    ...
    public void changeFeeOnAction(ActionEvent event) {
        ...
        if(newFee <= 0){
            ControllerUtil.showErrorMessage("Phí dịch vụ mới không hợp lệ, vui lòng nhập lại!");
            return;
        }
        boolean confirmed = ControllerUtil.showConfirmationDialog("Xác nhận thay đổi phí dịch vụ chung cư" , "Bạn có chắc chắn muốn thay đổi phí dịch vụ chung cư không ?");
        if(confirmed){
            MysqlConnector.getInstance().changeFeeData(tenPhi, newFee, Year.now().getValue());
            ControllerUtil.showSuccessAlert("Thay đổi phí dịch vụ chung cư thành công!");
            feeList = MysqlConnector.getInstance().getFeeData(tenPhi, yearCBox.getValue());
            feeTableView.setItems(feeList);
            feeTableView.refresh();
            feeLabel.setText("Phí dịch vụ chung cư hiện tại là: " + newFee + " /đồng/m2/tháng");
            changeFeeText.clear();
        }
    }
    ...
    private void loadData(int year) {
        ...
        feeLabel.setText("Phí dịch vụ chung cư hiện tại là: " + fee + " /đồng/m2/tháng");
    }
